...
export function StaffTopNav({ role, children }: { role: "counsellor" | "admin"; children?: React.ReactNode }) {
  ...
  return <div className="min-h-screen bg-background md:flex">
    <aside className="hidden w-64 shrink-0 border-r border-border-color bg-surface px-5 py-7 md:flex md:flex-col">
      <Link href={role === "counsellor" ? "/counsellor" : "/admin"} className="flex items-center gap-3 group">
        <SaathLogo className="h-8 w-auto transition-transform duration-200 group-hover:scale-105" size={32} />
        <span className="font-display text-3xl font-bold text-primary-teal">SAATH</span>
      </Link>
      ...
      <nav className="mt-8 flex flex-col gap-1.5" aria-label="Staff navigation">
        {items.map(({ href, label, icon: Icon }) => (
          <Link key={label} href={href} className={`flex items-center gap-3 rounded-2xl px-4 py-3 text-sm font-semibold transition-colors ${pathname === href ? "bg-primary-teal text-white shadow-xs" : "text-text-secondary hover:bg-surface-subtle hover:text-text-primary"}`}>
            <Icon size={18} />
            <span>{label}</span>
          </Link>
        ))}
      ...
      <div className="mt-auto pt-6">
        <Button size="sm" variant="ghost" className="w-full justify-start text-text-muted hover:text-error" onClick={signOut}>
